...
class TestFunctions(TestCase):
    ...
    def test_pdf_parser_extracts_correctly(self):
        self.assertEqual(re.sub(r"(?m)^\s+", "", pdf_parser.parse_pdf("src/tests/test_upload_files/one_page_test_file.pdf")), "test\n")
        self.assertEqual(re.sub(r"(?m)^\s+", "", pdf_parser.parse_pdf("src/tests/test_upload_files/two_page_test_file.pdf")), "test\npage 2\n")
